...
Evaluation and reevaluation also cannot self-certify newly attached artifacts. If a caller sends support artifacts or code-execution proof already marked `verification_status=verified`, Harness only preserves that trust when the artifact provenance is already canonical, such as `github/api` sync or `harness/manual_review|verification`. Otherwise Harness downgrades the artifact back to `unverified`, strips it from validated evidence, and forces canonical verification or reconciliation to re-attest it before it counts.
