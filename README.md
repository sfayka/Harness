...
Reevaluation also cannot self-certify newly attached code-execution artifacts. If a caller sends PR, commit, branch, or changed-file proof through `new_artifacts` already marked `verification_status=verified`, Harness downgrades it back to `unverified`, strips it from validated evidence, and forces canonical verification or reconciliation to re-attest it before it counts.
