...
- If that pruning empties `validated_artifact_ids`, Harness also clears caller-supplied `completion_evidence.status`, `validated_at`, and `validator` fields and returns the evidence state to `deferred`.
