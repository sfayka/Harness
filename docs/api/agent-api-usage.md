...
Like completion claims, evaluation overlays and reevaluation do not trust caller-submitted `verification_status=verified` by default. If a caller attaches support artifacts or code-execution proof already marked verified, Harness only preserves that trust when the artifact provenance is already canonical, such as `github/api` sync or `harness/manual_review|verification`. Otherwise it downgrades those artifacts back to `unverified`, strips them from validated evidence, and requires canonical verification or reconciliation to earn that trust again.
