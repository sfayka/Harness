...
- when recovery fails into `in_review`, Harness must also persist a canonical reconciliation `review_request` plus an append-only evaluation record so the manual-review gate is explicit and later resolvable
