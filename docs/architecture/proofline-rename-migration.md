...
| Symphony binary env vars | `PROOFLINE_SYMPHONY_BIN`, `HARNESS_SYMPHONY_BIN`, `SYMPHONY_BIN` | alias-first | Prefer the Proofline-named Symphony binary path when multiple names are present. Keep Harness-named and generic Symphony overrides valid as compatibility fallbacks. |
| Runtime/process env vars | `PROOFLINE_RUNTIME_*`, `HARNESS_RUNTIME_*` | alias-first | Prefer Proofline-named runtime metadata and executable overrides when both names are present. Keep Harness-named runtime variables valid as compatibility fallbacks. |
