...
- if `resume_target_status` is `dispatch_ready`, Harness immediately re-applies continuation policy so a real execution attempt is recorded through the current execution surface
- if `resume_target_status` is `assigned`, Harness restores the active assignment so work is no longer stranded behind a resolved clarification
