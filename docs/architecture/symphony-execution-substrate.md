...
These commands write a JSON summary to stdout. They use disposable local stores, do not start Symphony, do not read live Linear or GitHub state, and do not authorize task completion. Each summary includes `completion_validation_summary`; any legacy `accepted_completion` value is derived from that Proofline verdict, not from runner status or task lifecycle status alone. The `handoff` command renders the Symphony-compatible payload through the disabled transport boundary and marks `transport_status=disabled`, `dispatch_enabled=false`, `live_dispatch_enabled=false`, and `safe_to_execute_live=false`. They exist to make the execution-substrate boundary testable before Proofline connects a real Symphony runner.
